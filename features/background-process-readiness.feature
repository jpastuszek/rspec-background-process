Feature: Background process readiness verification
	...
	Scenario: Readiness check is reset to default for each scenario
		...
		And fresh test-ok process is running and ready
		Then test-ok process should be ready
		Then test-ok process log should contain hello world
		Then test-ok process log should not contain started
		When we remember test-ok process pid

	@readiness @reset
	Scenario: Readiness check is reset to default for each scenario
		And test-ok process is running and ready
		Then test-ok process log should contain started
		Then test-ok process pid should be as remembered
